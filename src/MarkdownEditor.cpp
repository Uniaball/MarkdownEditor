...
void MarkdownEditor::mouseMoveEvent(QMouseEvent *event)
{
    if (m_dragging && (event->buttons() & Qt::LeftButton)) {
        ...
        const int dragThresh = QApplication::startDragDistance() * 2;
        if (!m_hasDragConsent) {
            if (delta.manhattanLength() >= dragThresh) {
                m_hasDragConsent = true;
